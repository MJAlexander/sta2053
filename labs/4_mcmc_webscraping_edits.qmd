...
Normal prior on $\mu$
$$
\mu \sim N(\mu_0, \sigma^2_{\mu_0})
$$
Gamma prior on $1/\sigma^2$
$$
...
for(s in 2:S) {
  ...
  var_post <- 1/(1/sigma2_mu0 + n*(phi_sp[s-1,2]))
  mu_post <- (mu0/sigma2_mu0 + n*mean_y*phi_sp[s-1,2])*var_post
  mu_next <- rnorm(1, mean = mu_post, sd = sqrt(var_post))
  # generate a new 1/sigma^2 value from its full conditional
  s2n <- (n-1)*var_y + n*(mean_y - mu_next)^2
  sigma_n_post <- 1/nun*(nu0*sigma2_0+s2n)
  tau_next <- rgamma(1, nun/2, nun/2*sigma_n_post)
  
  phi <- c(mu_next, tau_next)
...
```{r}
as_tibble(phi_sp) |> 
  mutate(iteration = 1:S) |> 
  ggplot(aes(iteration, V1)) +
  geom_line()
```
