...
moda <- stan(data = stan_data,
             file = here("code/models/lka_linear_me_plus.stan"))

modb <- stan(data = stan_data,
             file = here("code/models/lka_linear_sd.stan"))
...
ggplot(lka, aes(year, logit_ratio)) +
  ...
  geom_line(data = res, aes(year, .value)) + 
  geom_ribbon(data = res, aes(y = .value, ymin = .lower, ymax = .upper), alpha = 0.2)+
  geom_line(data = resa, aes(year, .value), color = "red") + 
  geom_ribbon(data = resa, aes(y = .value, ymin = .lower, ymax = .upper), alpha = 0.2, fill = "red")+
  geom_line(data = resb, aes(year, .value), color = "purple") + 
  geom_ribbon(data = resb, aes(y = .value, ymin = .lower, ymax = .upper), alpha = 0.2, fill = "purple")+
  theme_bw()+
  labs(title = "Ratio of neonatal to under-five child mortality (logit), Sri Lanka",
       y = "logit ratio", subtitle = "Linear fit shown in black, linear fit with addition sd shown in red")
```
...
```{r}
mod3 <- stan(data = stan_data,
             file = here("code/models/lka_rw2.stan"))
...
ggplot(lka, aes(year, logit_ratio)) +
  ...
  geom_ribbon(aes(ymin = logit_ratio - se, 
                  ...
                  fill =  source), alpha = 0.1) + 
  theme_bw()+
  geom_line(data = res3 |> filter(year<2015), aes(year, .value)) + 
  geom_ribbon(data = res3 |> filter(year<2015), aes(y = .value, ymin = .lower, ymax = .upper), alpha = 0.2)+
  theme_bw()+
  labs(title = "Ratio of neonatal to under-five child mortality (logit), Sri Lanka",
       y = "logit ratio", subtitle = "Random walk fit shown in black")
```
