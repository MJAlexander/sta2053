...
sum((ds$sex=="M"))/nrow(ds)
sum(ds$ilive=="Y")/nrow(ds)
```
...
ds <- ds |> 
  mutate(log_gest = log(gest),
         log_gest_cs = (log(gest) - mean(log(ds$gest)))/sd(log(ds$gest)))
...
beta_0 <- mod1 |> 
  gather_draws(beta[k]) |> 
  mutate(variable = paste(.variable, k, sep = "_"),
         value = .value) |> 
  ungroup() |> 
...
intercept_slope <- beta_0 |> 
  ...
  select(-variable) |> 
  bind_cols(beta_1 |> rename(slope = value) |> select(-variable)) 
